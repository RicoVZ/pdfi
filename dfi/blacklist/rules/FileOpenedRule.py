...
class FileOpenedRule(Rule):

	def match_rule(self, rule_data):

		matches = []

		for file_opened in self.cuckoo_report.file_opened:

			try:
				if self.search_wildcard_regex(rule_data, file_opened):
					matches.append(file_opened)
